...
class deepLearningPlayer(BasePokerPlayer):
    my_uuid = ""
    ...
    def __init__(self, load_weights=True):
        ...
        self.target_Q = [[0, 0, 0]]
        self.model = self.keras_model(load_weights)

    def declare_action(self, valid_actions, hole_card, round_state):

        def get_card_x(card):
            suit = card[0]
            return deepLearningPlayer.suits[suit]

        def get_card_y(card):
            small_or_big_blind_turn = card[1]
            return deepLearningPlayer.ranks[small_or_big_blind_turn]
        ...
        def record_state():
            ...
            if self.has_played:
                reward_sb = deepLearningPlayer.y * np.max(self.cur_Q_values)
                ...
                if len(self.prev_round_features) > deepLearningPlayer.max_replay_size:
        ...
        def pick_action():
            ...
            if action == "raise":
                ...
                amount = min_raise if min_raise == max_raise else np.random.randint(min_raise, max_raise + 1)
        ...
        preflop_actions = convert_to_image_grid(deepLearningPlayer.starting_stack, round_state, 'preflop')

        if round_state['street'] == 'flop':
            flop = round_state['community_card']
            flop_cards_img = get_street_grid(flop)
            flop_actions = convert_to_image_grid(deepLearningPlayer.starting_stack, round_state, 'flop')

        if round_state['street'] == 'turn':
            turn = round_state['community_card'][3]
            turn_cards_img = get_street_grid([turn])
            turn_actions = convert_to_image_grid(deepLearningPlayer.starting_stack, round_state, 'turn')

        if round_state['street'] == 'river':
            river = round_state['community_card'][4]
            river_cards_img = get_street_grid([river])
            river_actions = convert_to_image_grid(deepLearningPlayer.starting_stack, round_state, 'river')
    ...
    def receive_game_start_message(self, game_info):
        deepLearningPlayer.my_uuid = self.uuid

    def receive_round_start_message(self, round_count, hole_card, seats):
        for seat in seats:
            if deepLearningPlayer.my_uuid == seat["uuid"]:
                deepLearningPlayer.my_starting_stack = seat["stack"]
            else:
                deepLearningPlayer.opp_starting_stack = seat["stack"]
    ...
    def receive_round_result_message(self, winners, hand_info, round_state):
        def get_real_reward():
            if winners[0]['uuid'] == deepLearningPlayer.my_uuid:
                return winners[0]['stack'] - deepLearningPlayer.my_starting_stack
            else:
                return -(winners[0]['stack'] - deepLearningPlayer.opp_starting_stack)
        ...
        if self.action_sb == 0:
            ...
            if np.argmax(self.target_Q) != 0:
                self.target_Q[0, self.action_sb] = reward * deepLearningPlayer.y
    ...
    def save_weights(self):
        self.model.save_weights("Group18.weights.h5")
        print("✅ Weights saved to Group18.h5")

    def keras_model(self, load_weights):
        input_cards = Input(shape=(4,13,4), name="cards_input")
        input_actions = Input(shape=(2,6,4), name="actions_input")
        input_position = Input(shape=(1,), name="position_input")
        ...
        if load_weights:
            try:
                model.load_weights('Group18.weights.h5')
                print("✅ Weights loaded from Group18.h5")
...
def setup_ai():
    return deepLearningPlayer(load_weights=True)  # Set to False when training
