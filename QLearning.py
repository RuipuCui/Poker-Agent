...
class QLearningPlayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        def get_card_x(card):
            suit = card[0]
            return QLearningPlayer.suits[suit]

        def get_card_y(card):
            small_or_big_blind_turn = card[1]
            return QLearningPlayer.ranks[small_or_big_blind_turn]
        ...
        def record_state():
            ...
            if self.has_played:
                reward_sb = QLearningPlayer.y * np.max(self.cur_Q_values)
                ...
                if len(self.prev_round_features) > QLearningPlayer.max_replay_size:
        ...
        def pick_action():
            if np.random.rand(1) < QLearningPlayer.e:
        ...
        preflop_actions = convert_to_image_grid(QLearningPlayer.starting_stack, round_state, 'preflop')

        if round_state['street'] == 'flop':
            flop = round_state['community_card']
            flop_cards_img = get_street_grid(flop)
            flop_actions = convert_to_image_grid(QLearningPlayer.starting_stack, round_state, 'flop')

        if round_state['street'] == 'turn':
            turn = round_state['community_card'][3]
            turn_cards_img = get_street_grid([turn])
            turn_actions = convert_to_image_grid(QLearningPlayer.starting_stack, round_state, 'turn')

        if round_state['street'] == 'river':
            river = round_state['community_card'][4]
            river_cards_img = get_street_grid([river])
            river_actions = convert_to_image_grid(QLearningPlayer.starting_stack, round_state, 'river')
    ...
    def receive_round_start_message(self, round_count, hole_card, seats):
        for seat in seats:
            if QLearningPlayer.my_uuid == seat["uuid"]:
                QLearningPlayer.my_starting_stack = seat["stack"]
            else:
                QLearningPlayer.opp_starting_stack = seat["stack"]
    ...
    def receive_round_result_message(self, winners, hand_info, round_state):
        def get_real_reward():
            if winners[0]['uuid'] == QLearningPlayer.my_uuid:
                return winners[0]['stack'] - QLearningPlayer.my_starting_stack
            else:
                return -(winners[0]['stack'] - QLearningPlayer.opp_starting_stack)
        ...
        if self.action_sb == 0:
            ...
            if np.argmax(self.target_Q) != 0:
                self.target_Q[0, self.action_sb] = reward * QLearningPlayer.y
